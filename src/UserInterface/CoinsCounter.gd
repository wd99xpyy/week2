...
func useCoin(amount):#QW change the amount of the coins
	coins_collected -= amount#QW
	coins_label.set_text(str(coins_collected))#QW
	
func getCoinAmount():#QW get the amount of the coins
	return str(coins_collected)#QW
